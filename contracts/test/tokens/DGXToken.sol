...
contract DGX_Ownable {
  ...
  /**
   * @dev The DGX_Ownable constructor sets the original `owner` of the contract to the sender
   * account.
   */
  function DGX_Ownable() public {
    owner = msg.sender;
  }
...
contract DGX_Pausable is DGX_Ownable {
  event Pause();
  event Unpause();
...
contract DGX_CanReclaimToken is DGX_Ownable {
  using SafeERC20 for DGX_ERC20Basic;

  /**
   * @dev Reclaim all DGX_ERC20Basic compatible tokens
   * @param token DGX_ERC20Basic The address of the token contract
   */
  function reclaimToken(DGX_ERC20Basic token) external onlyOwner {
...
contract DGX_Claimable is DGX_Ownable {
...
contract DGX_AddressList is DGX_Claimable {
    string public name;
    mapping (address => bool) public onList;

    function DGX_AddressList(string _name, bool nullValue) public {
        name = _name;
...
contract DGX_HasNoContracts is DGX_Ownable {

  /**
   * @dev Reclaim ownership of DGX_Ownable contracts
   * @param contractAddr The address of the DGX_Ownable to be reclaimed.
   */
  function reclaimContract(address contractAddr) external onlyOwner {
    DGX_Ownable contractInst = DGX_Ownable(contractAddr);
    contractInst.transferOwnership(owner);
  }
}

contract DGX_HasNoEther is DGX_Ownable {
  ...
  */
  function DGX_HasNoEther() public payable {
    require(msg.value == 0);
  }
...
contract DGX_HasNoTokens is DGX_CanReclaimToken {
...
contract DGX_NoOwner is DGX_HasNoEther, DGX_HasNoTokens, DGX_HasNoContracts {
}

contract DGX_AllowanceSheet is DGX_Claimable {
    using DGX_SafeMath for uint256;
...
contract DGX_BalanceSheet is DGX_Claimable {
    using DGX_SafeMath for uint256;
...
contract DGX_BasicToken is DGX_ERC20Basic, DGX_Claimable {
  using DGX_SafeMath for uint256;

  DGX_BalanceSheet public balances;

  uint256 totalSupply_;

  function setBalanceSheet(address sheet) external onlyOwner {
    balances = DGX_BalanceSheet(sheet);
  ...
  */
  ...
  function transferAllArgsNoAllowance(address _from, address _to, uint256 _value) internal {
    ...
    // DGX_SafeMath.sub will throw if there is not enough balance.
...
contract DGX_BurnableToken is DGX_BasicToken {
...
contract DGX_ERC20 is DGX_ERC20Basic {
...
library SafeERC20 {
  function safeTransfer(DGX_ERC20Basic token, address to, uint256 value) internal {
    assert(token.transfer(to, value));
  }

  function safeTransferFrom(DGX_ERC20 token, address from, address to, uint256 value) internal {
    assert(token.transferFrom(from, to, value));
  }

  function safeApprove(DGX_ERC20 token, address spender, uint256 value) internal {
    assert(token.approve(spender, value));
  }
}

contract DGX_StandardToken is DGX_ERC20, DGX_BasicToken {

  DGX_AllowanceSheet public allowances;

  function setAllowanceSheet(address sheet) external onlyOwner {
    allowances = DGX_AllowanceSheet(sheet);
...
contract DGX_CanDelegate is DGX_StandardToken {
    ...
    DGX_DelegateERC20 public delegate;

    event DelegatedTo(address indexed newContract);

    // Can undelegate by passing in newContract = address(0)
    function delegateToNewContract(DGX_DelegateERC20 newContract) public onlyOwner {
...
contract DGX_StandardDelegate is DGX_StandardToken, DGX_DelegateERC20 {
...
contract DGX_PausableToken is DGX_StandardToken, DGX_Pausable {
...
contract DigixGoldToken is DGX_StandardDelegate, DGX_PausableToken, DGX_BurnableToken, DGX_NoOwner, DGX_CanDelegate {
    string public name = "DigixGoldToken";
    string public symbol = "DGX";
    uint8 public constant decimals = 9;

    DGX_AddressList public canReceiveMintWhiteList;
    DGX_AddressList public canBurnWhiteList;
    DGX_AddressList public blackList;
    DGX_AddressList public noFeesList;
    uint256 public burnMin = 10000 * 10**uint256(decimals);
    uint256 public burnMax = 20000000 * 10**uint256(decimals);

    uint80 public transferFeeNumerator = 13;
    ...
    function DigixGoldToken() public {
        totalSupply_ = 0;
        staker = msg.sender;
    }

    function setLists(DGX_AddressList _canReceiveMintWhiteList, DGX_AddressList _canBurnWhiteList, DGX_AddressList _blackList, DGX_AddressList _noFeesList) onlyOwner public {
