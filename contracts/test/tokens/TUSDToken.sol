...
contract TUSD_Ownable {
  ...
  /**
   * @dev The TUSD_Ownable constructor sets the original `owner` of the contract to the sender
   * account.
   */
  function TUSD_Ownable() public {
    owner = msg.sender;
  }
...
contract TUSD_Pausable is TUSD_Ownable {
  event Pause();
  event Unpause();
...
contract TUSD_CanReclaimToken is TUSD_Ownable {
  using SafeERC20 for TUSD_ERC20Basic;

  /**
   * @dev Reclaim all TUSD_ERC20Basic compatible tokens
   * @param token TUSD_ERC20Basic The address of the token contract
   */
  function reclaimToken(TUSD_ERC20Basic token) external onlyOwner {
...
contract TUSD_Claimable is TUSD_Ownable {
...
contract TUSD_AddressList is TUSD_Claimable {
    string public name;
    mapping (address => bool) public onList;

    function TUSD_AddressList(string _name, bool nullValue) public {
        name = _name;
...
contract TUSD_HasNoContracts is TUSD_Ownable {

  /**
   * @dev Reclaim ownership of TUSD_Ownable contracts
   * @param contractAddr The address of the TUSD_Ownable to be reclaimed.
   */
  function reclaimContract(address contractAddr) external onlyOwner {
    TUSD_Ownable contractInst = TUSD_Ownable(contractAddr);
    contractInst.transferOwnership(owner);
  }
}

contract TUSD_HasNoEther is TUSD_Ownable {
  ...
  */
  function TUSD_HasNoEther() public payable {
    require(msg.value == 0);
  }
...
contract TUSD_HasNoTokens is TUSD_CanReclaimToken {
...
contract TUSD_NoOwner is TUSD_HasNoEther, TUSD_HasNoTokens, TUSD_HasNoContracts {
}

contract TUSD_AllowanceSheet is TUSD_Claimable {
    using TUSD_SafeMath for uint256;
...
contract TUSD_BalanceSheet is TUSD_Claimable {
    using TUSD_SafeMath for uint256;
...
contract TUSD_BasicToken is TUSD_ERC20Basic, TUSD_Claimable {
  using TUSD_SafeMath for uint256;

  TUSD_BalanceSheet public balances;

  uint256 totalSupply_;

  function setBalanceSheet(address sheet) external onlyOwner {
    balances = TUSD_BalanceSheet(sheet);
  ...
  */
  ...
  function transferAllArgsNoAllowance(address _from, address _to, uint256 _value) internal {
    ...
    // TUSD_SafeMath.sub will throw if there is not enough balance.
...
contract TUSD_BurnableToken is TUSD_BasicToken {
...
contract TUSD_ERC20 is TUSD_ERC20Basic {
...
library SafeERC20 {
  function safeTransfer(TUSD_ERC20Basic token, address to, uint256 value) internal {
    assert(token.transfer(to, value));
  }

  function safeTransferFrom(TUSD_ERC20 token, address from, address to, uint256 value) internal {
    assert(token.transferFrom(from, to, value));
  }

  function safeApprove(TUSD_ERC20 token, address spender, uint256 value) internal {
    assert(token.approve(spender, value));
  }
}

contract TUSD_StandardToken is TUSD_ERC20, TUSD_BasicToken {

  TUSD_AllowanceSheet public allowances;

  function setAllowanceSheet(address sheet) external onlyOwner {
    allowances = TUSD_AllowanceSheet(sheet);
...
contract TUSD_CanDelegate is TUSD_StandardToken {
    ...
    TUSD_DelegateERC20 public delegate;

    event DelegatedTo(address indexed newContract);

    // Can undelegate by passing in newContract = address(0)
    function delegateToNewContract(TUSD_DelegateERC20 newContract) public onlyOwner {
...
contract TUSD_StandardDelegate is TUSD_StandardToken, TUSD_DelegateERC20 {
...
contract TUSD_PausableToken is TUSD_StandardToken, TUSD_Pausable {
...
contract TrueUSD is TUSD_StandardDelegate, TUSD_PausableToken, TUSD_BurnableToken, TUSD_NoOwner, TUSD_CanDelegate {
    string public name = "TrueUSD";
    string public symbol = "TUSD";
    uint8 public constant decimals = 18;

    TUSD_AddressList public canReceiveMintWhiteList;
    TUSD_AddressList public canBurnWhiteList;
    TUSD_AddressList public blackList;
    TUSD_AddressList public noFeesList;
    ...
    function setLists(TUSD_AddressList _canReceiveMintWhiteList, TUSD_AddressList _canBurnWhiteList, TUSD_AddressList _blackList, TUSD_AddressList _noFeesList) onlyOwner public {
